...
- **URL:** `https://event.sumit.indevs.in/admin/index.html`  
...
| **`sumit.indevs.in` / `event.sumit.indevs.in`** | Main portfolio site (event preferred for admin) | Your existing Pages / DNS setup |
...
**Practical default:** use **`https://event.sumit.indevs.in/admin/index.html`** directly and skip the `admin` subdomain until DNS + GitHub or Cloudflare are configured.
...
2. **Settings** → **Variables**: set **`ALLOWED_ORIGINS`** to include every origin that loads the admin UI, e.g. `https://event.sumit.indevs.in`, `https://sumit.indevs.in`, `https://admin.sumit.indevs.in` (see `wrangler.toml`).
