...
3. Open **`https://sumit.indevs.in/admin/index.html`** (or `https://admin.sumit.indevs.in/` after DNS) → **Login** in the ribbon → use the sidebar and inspector. No manual `?admin_edit=1` in the address bar.

### Optional: standalone homepage

1. Log in via `https://sumit.indevs.in/dev/index.html` → **Login (GitHub)**.
2. Open `https://sumit.indevs.in/pages/homepage.html?admin_edit=1` if you want the old floating bar only (adjust host if needed).
